...
static void pwm0_update_period_and_sync(uint32_t target_hz) {
    ...
    if (pinsocket_pwm_inited) {
        for (uint8_t ch = 0; ch < 4U; ch++) {
            float duty = pinsocket_channel_enabled[ch] ? pinsocket_last_duty[ch] : 0.0f;
...
void board_init_imuheater(void) {
    ...
    init_imu_heater_pwm_pin(); /* PY01 -> PWM1 CH5 */
...
bool board_get_user_key_status(void) { return gpio_read_pin(HPM_GPIO0, GPIO_DI_GPIOY, 3) == 0; }

void board_init_user_sw(void) { init_user_sw_pin(); }

bool board_get_user_sw_status(void) { return gpio_read_pin(HPM_GPIO0, GPIO_DI_GPIOY, 4) != 0; }
